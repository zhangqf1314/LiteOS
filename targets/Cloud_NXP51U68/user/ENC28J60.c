...
extern status_t SPI_MasterTransferBlockingByte(SPI_Type *base, spi_transfer_t *xfer);
extern uint8_t SPI_MasterTransferBlockingByte_new(SPI_Type *base, spi_transfer_t *xfer);
...
unsigned char SPI_ReadWrite(unsigned char writedat,int end)
{
    ...
    v8_r =SPI_MasterTransferBlockingByte_new(EXAMPLE_SPI_MASTER_BASEADDR,&transfer);    
	//LOS_IntUnLock();
    return  v8_r;
}
